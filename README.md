...
| # | Правило | Что делает |
|---|---------|------------|
| 1 | `TransliterationRule` | Любой не-ASCII символ → ASCII; разделители пути (`/`, `\`) и управляющие символы, которые может породить транслитерация (`½`→`1/2`, `∖`→`\`, U+2028→перевод строки), заменяются на `-`, чтобы имя осталось одним компонентом пути |
