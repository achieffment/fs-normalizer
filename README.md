...
- Список исключений (`exclude.txt` в корне проекта): объекты, чей путь содержит указанный фрагмент, не нормализуются; без непустого `include.txt` внутрь исключённых каталогов не заходим, и такие объекты не учитываются в подсчёте (см. «Исключения» и «Повторное включение»).
...
- **Одинаково на Windows, WSL, Linux и macOS.** Разделители `\` и `/` равнозначны; **регистр сегментов учитывается** — паттерн `Archive` совпадёт с `.../Archive/...`, но не с `.../archive/...` (указывайте регистр как на диске). Исключение — буква диска: `D:\` и `/mnt/d/` (сегмент `mnt` — литерал, только lowercase) сводятся к одному токену `d`, поэтому `D:\Programs` и `/mnt/d/Programs` совпадают в обе стороны, но **разные диски не путаются** — `E:\Programs` не исключит `D:\Programs`. Фрагмент без диска (`Programs`) совпадает на любом диске; `Programs` ≠ `programs`.
- **Исключённое не считается.** Пропущенные объекты не попадают в итоговый счётчик (ни в «переименовано», ни в «пропущено»). Без непустого `include.txt` внутрь исключённого каталога не заходим и его содержимое не трогаем; при непустом `include.txt` обход продолжается — внутри исключённого могут быть повторно включённые потомки (см. «Повторное включение»).
- **Glob-шаблоны (спецсимвол только `*`).** Звёздочка `*` означает любое количество символов (в том числе ноль) **внутри одного сегмента** — она не пересекает разделитель: `*.txt` (сегмент, оканчивающийся на `.txt`), `a*` (сегмент на `a`), `*foo*` (содержит `foo`), `data-*-final`. Glob **регистрозависим**: `*.txt` не совпадёт с `Notes.TXT`. Одиночная `*` — это ровно один сегмент целиком (`Projects/*/build`). Последовательность `**` **как целый сегмент** означает ноль или более сегментов (`Projects/**/Data`); в любой другой комбинации со звёздами в сегменте (`a**b`, `***`) лишние звёзды схлопываются в обычный intra-segment `*`. Символы `?`, `[`, `]` — **литералы**, а не метасимволы glob: паттерн `Файл [1]` совпадает буквально с сегментом `Файл [1]` (имена со скобками в проекте встречаются повсюду). Те же шаблоны работают и в `include.txt`.
- **Без файла — обычное поведение.** Если `exclude.txt` отсутствует или пуст, никакие проверки не выполняются.

> На регистронезависимой ФС (Windows, macOS по умолчанию) `os.walk` возвращает имена в том регистре, в котором они хранятся на диске — паттерн в `exclude.txt`/`include.txt` должен совпадать с этим регистром.
...
- **Полный обход при непустом include.** Пока в `include.txt` есть хотя бы один паттерн, исключённые каталоги не обрезаются при обходе: утилита заходит внутрь, чтобы найти повторно включённых потомков. Сами исключённые объекты по-прежнему не нормализуются, пока include их явно не вернёт.
